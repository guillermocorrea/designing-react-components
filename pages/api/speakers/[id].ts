import { NextApiRequest, NextApiResponse } from "next";
import path from "path";
import fs from "fs";
import { promisify } from "util";

const readFileAsync = promisify(fs.readFile);
const writeFileAsync = promisify(fs.writeFile);

const delay = (ms: number) => new Promise((resolve) => setTimeout(resolve, ms));

export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse
) {
  const method = req.method;

  switch (method) {
    case "GET":
      await handleGetMethod(req, res);
      break;
    case "PUT":
      await handlePutMethod(req, res);
      break;
    case "DELETE":
      await handleDeleteMethod(req, res);
      break;
    default:
      res.status(501).send(`Method ${method} not implemented`);
      console.log(`Method ${method} not implemented`);
  }
}

async function handlePutMethod(req: NextApiRequest, res: NextApiResponse) {
  const jsonFile = path.resolve("./", "db.json");
  const id = req.query.id as string;
  try {
    const file = await readFileAsync(jsonFile);
    await delay(1000);
    const speakers = JSON.parse(file as unknown as string);
    if (speakers) {
      const body = req.body;
      const newSpeakersData = speakers.map((speaker) =>
        speaker.id === id ? body : speaker
      );
      await writeFileAsync(jsonFile, JSON.stringify(newSpeakersData, null, 2));
      res.status(200).json(speakers);
      console.log(`PUT /api/speakers/${id} status: 200`);
    } else {
      res.status(500).send("No data found");
      console.log(`PUT /api/speakers/${id} status: 500`);
    }
  } catch (error) {
    console.error(error);
    res.status(500).send(error);
    console.log(`PUT /api/speakers/${id} status: 500`);
  }
}

async function handleDeleteMethod(req: NextApiRequest, res: NextApiResponse) {
  const jsonFile = path.resolve("./", "db.json");
  const id = req.query.id as string;
  try {
    const file = await readFileAsync(jsonFile);
    await delay(1000);
    const speakers = JSON.parse(file as unknown as string);
    if (speakers) {
      const newSpeakersData = speakers.filter((speaker) => speaker.id !== id);
      await writeFileAsync(jsonFile, JSON.stringify(newSpeakersData, null, 2));
      res.status(200).json(speakers);
      console.log(`DELETE /api/speakers/${id} status: 200`);
    } else {
      res.status(500).send("No data found");
      console.log(`DELETE /api/speakers/${id} status: 500`);
    }
  } catch (error) {
    console.error(error);
    res.status(500).send(error);
    console.log(`DELETE /api/speakers/${id} status: 500`);
  }
}

async function handleGetMethod(_: NextApiRequest, res: NextApiResponse) {
  const jsonFile = path.resolve("./", "db.json");
  try {
    const file = await readFileAsync(jsonFile);
    await delay(1000);
    const speakers = JSON.parse(file as unknown as string);
    if (speakers) {
      res.status(200).json(speakers);
      console.log("GET /api/speakers/ status: 200");
    } else {
      res.status(500).send("No data found");
      console.log("GET /api/speakers/ status: 500");
    }
  } catch (error) {
    console.error(error);
    res.status(500).send(error);
    console.log("GET /api/speakers/ status: 500");
  }
}
